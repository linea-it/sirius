...
export default class TableComponents extends Component {
  constructor() {
    super();
    const columnsComponents = columnsTableComponents;
    ...
    for (const col of columnsComponents) {
  ...
  render() {
    const header = (
      <div style={{ textAlign: 'left' }}>
        <MultiSelect
          value={this.state.colsComponents}
          options={this.colOptionsComponents}
          onChange={this.onColumnToggleComponents}
        />
        <br />
        <InputText
          type="search"
    ...
    const columnsComponents = this.state.colsComponents.map(col => {
      return (
        <Column
          key={col.field}
          field={col.field}
